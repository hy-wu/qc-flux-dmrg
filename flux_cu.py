...
class Flux:
    def __init__(self, E_Jb=7.5e9, C_Jb=40e-15, C_J=32.9e-15,
        ...
        self.R_bond = np.array([0, 1, np.exp(1j * phi_ext).get(), np.exp(-1j * phi_ext).get()]).reshape((1, 4, 1))
    ...
    def init_MPS(self):
        ...
        def rq(A):
            ...
            R, Q= np.linalg.qr(np.flip(A).T)
            return np.flip(Q).T, np.flip(R).T

        def Mps_LQP(T, UR):
            """ (0-T-L)(0-UR-1) -> (0-UL-1)(0-T1-L) """
            UL, T1 = rq(np.tensordot(T, UR, (-1, 0)).reshape((T.shape[0], -1)))  # , mode='economic')
    ...
    def optimizeT_site(self, site_: int):
        ...
        if self.usingLanczos:  # using block Lanczos
            ...
            #         for _ in range(self.N_Lanczos)]
            Alpha = [np.zeros((self.N_Lanczos, g, g), dtype=complex) for _ in range(self.N_Lanczos)]
            Beta = [np.zeros((self.N_Lanczos, g, g), dtype=complex) for _ in range(self.N_Lanczos)]
            ...
            for n in range(self.N_Lanczos):
                ...
                #                                [11, 12]))
                Alpha[n] = np.einsum(HPsi, [1, 2, 3, 12],
                                        np.conj(Psi[n]), [1, 2, 3, 11],
                                        [11, 12])
                ...
                if n < self.N_Lanczos - 1:
                    ...
                    if n:
                        ...
                        Beta[n] = np.conj(Vh.T) @ np.diag(D) @ Vh
                        M[n * g:g + n * g, n * g - g:n * g] = Beta[n]
                        M[n * g - g:n * g, n * g:g + n * g] = np.conj(Beta[n].T)

                        # step E
                        Psi[n + 1] -= np.tensordot(Psi[n - 1], np.conj(Beta[n].T), (-1, 0))
            ...
            Eig, V = linalg.eigsh(M, k=g, which='SA')
            # Eig, V = la.eigh(M, eigvals=(0, g - 1))
            ...
            Eig, V = linalg.eigsh(A, k=self.N_excited, which='SA')
            # Eig, V = la.eigh(A, eigvals=(0, self.N_excited - 1))
