...
class Flux:
    ...
    def optimizeT_site(self, site_: int):
        ...
        if self.usingLanczos:   # using block Lanczos
            ...
            for n in range(self.N_Lanczos):
                ...
                # [-   Psi[n]  - ]
                #         \
                HPsi = Sub.NCon([H, Psi[n]], [[-1, -2, -3, 1, 2, 3], [1, 2, 3, -4]])

                # Alpha_n = <Psi_n|H|Psi_n>
                Alpha[n] = np.matrix(Sub.NCon([HPsi, np.conj(Psi[n])], [[1, 2, 3, -2], [1, 2, 3, -1]]))
    ...
    def optimizeT(self, usingLanczos: bool, Eng_criterion: float, N_criterion: int, Quiet: bool):
        ...
        for r in range(1000):
            ...
            if not Quiet:
                print(np.round(Eng1[1][:N_criterion] - Eng0[1][:N_criterion], 3), end='\r')
